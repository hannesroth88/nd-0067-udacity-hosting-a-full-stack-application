...
app
  ...
  .delete(async (req, res, next) => {
    try {
      const { id } = req.params
      const results = await db.query("DELETE FROM restaurants WHERE id = $1 returning id", [id])
      res.status(201).json({
        status: "success",
        data: results.rows[0]
      })
...
app.listen(PORT, () => console.log(`ExpressServer on PORT:${PORT},   Using DB: ${process.env.RDS_HOSTNAME}`))
